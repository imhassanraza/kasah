...
function send_mail_func($to = '',$subject,$message){
    ...
    $cc = "";
    $bcc = "akhzarjaved1993@gmail.com, mmuzammil45@gmail.com";
    $CI->email->from('noreply.explorelogics@gmail.com', 'Explore Logics');
    if (!empty($to) && $to != 'support@kasah.co') {         //&& $to != 'support@kasah.co'
        $CI->email->to($to);
    }else{
        $CI->email->to('akhzarjaved1993@gmail.com, mmuzammil45@gmail.com,sana.baig96@gmail.com');
    }
    $CI->email->reply_to('noreply.explorelogics@gmail.com', 'Explore Logics');
    if (!empty($cc)){
       $CI->email->cc($cc);
   }
   if (!empty($bcc)){
