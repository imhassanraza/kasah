...
class Admin_model extends CI_Model {
	...
	public function login_credential($username,$pass){
		$hash_pass="password('".$pass."')"; 
		$username="'".$username."'";
		$query=$this->db->query("SELECT * FROM admins WHERE username= $username AND password = $hash_pass AND (role='super_admin' OR is_delete = 'N')");
		if($query->num_rows()>0){
			return true;
		}else{
			return false;
