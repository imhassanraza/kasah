...
if (!function_exists('get_categories')) {
	function get_categories($type) {
		...
		$CI->db->where('status', 1);
		$CI->db->where_in('for_free_product', $type);
...
if (!function_exists('get_main_categories')){
	function get_main_categories() {
		...
		$CI->db->where('categories.parent_id', '0');
...
if (!function_exists('have_products')){
	function have_products($category_id) {
		$CI = & get_instance();
		$CI->db->select('*');
		$CI->db->from('products');
		$CI->db->where('category_id', $category_id);
		if (get_session('user_type') == 1) {
			$CI->db->where('can_see', 1);
		}
		$CI->db->where('status', 1);
		$query = $CI->db->get();
		return $query->num_rows();
...
if (!function_exists('get_categories_have_product_old')){
	function get_categories_have_product_old() {
		$CI = & get_instance();
		$CI->db->select('*');
		$CI->db->from('categories');
		if (get_session('user_type') == 1) {
...
if (!function_exists('get_all_resellers')) {
	function get_all_resellers() {
		$CI = & get_instance();
		$CI->db->select('id, username, status');
		$CI->db->from('admin_users');
		$CI->db->where('type', 1);
		return $CI->db->get()->result_array();
	}
...
if (!function_exists('check_subcategories')) {
	function check_subcategories($id) {
		$CI = & get_instance();
		$CI->db->select('*');
		$CI->db->from('categories');
		$CI->db->where('parent_id', $id);
		...
		$category_have_product = 0;
		foreach($result as $res) {
			$CI->db->select('*');
			$CI->db->from('products');
			$CI->db->where('category_id', $res['id']);
			if (get_session('user_type') == 1) {
				$CI->db->where('can_see', 1);
			}
			$CI->db->where('status', 1);
			$query = $CI->db->get();
			if($query->num_rows() > 0) {
				$category_have_product = 1;
			}
		}
		if ($category_have_product == 1) {
			return true;
		}
	}
}

if (!function_exists('get_subcategory_by_slug')){
	function get_subcategory_by_slug($id) {
		$CI = & get_instance();
		$CI->db->select('*');
		$CI->db->from('categories');
		if (get_session('user_type') == 1) {
		...
		$CI->db->where('categories.status', 1);
		$CI->db->where('categories.parent_id', $id);
		if (get_session('user_type') == 1) {
			$CI->db->where('categories.can_see', 1);
		}
		$CI->db->order_by('categories.sorting', 'ASC');
...
if (!function_exists('check_qty_alert')) {
	function check_qty_alert($product_id) {
		$CI = & get_instance();
		$CI->db->select('is_active_alert_qty');
		$CI->db->from('products');
		$CI->db->where('id', $product_id);
		$query = $CI->db->get()->row_array();
		return $query['is_active_alert_qty'];
	}
}
